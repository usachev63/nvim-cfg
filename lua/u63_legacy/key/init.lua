...
local layout_api = require 'u63_legacy.key.layout_api'
local langmapper = require 'u63_legacy.key.langmapper'
local user_maps = require 'u63_legacy.key.user_maps'
...
function M.init()
  ...
  if options.key.enable_langmapper then
    ...
    if not layout_api.get_layout then
      error(
        '[u63_legacy.key] Cannot use langmapper.nvim without layout_api backend!',
        3
      )
    end
  end
